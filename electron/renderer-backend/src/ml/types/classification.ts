...
const loadModel = async () => {
  ...
  logger.time("loadModel classification");
  net = await mobilenet.load();
  logger.timeEnd("loadModel classification");
};
 ...
 */
export const getClassificationIds = async (img) => {
  if (!net) await loadModel();

  let imageNetClassNumbers = [];

  try {
    const logits = await net.infer(img);
